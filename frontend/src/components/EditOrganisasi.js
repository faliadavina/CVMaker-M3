...
const EditDataOrganisasi = () => {
    ...
    useEffect(() => {
        const fetchDataOrganisasi = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/organisasi-by-id-akun/${id}`);
    ...
    const handleSelectOrganization = (organisasi) => {
        if (selectedOrganization && selectedOrganization.id_org === organisasi.id_org) {
    ...
    const handleUpdateClick = async () => {
        ...
        setIsEditing(true);
        try {
            await axios.patch(`http://localhost:5000/organisasi/${selectedOrganization.id_org}`, {
    ...
    const handleDeleteClick = async () => {
        if (selectedOrganization && selectedOrganization.id_org !== undefined) {
            const confirmation = window.confirm(`Apakah Anda yakin akan menghapus data organisasi ${selectedOrganization.nama_organisasi}?`);

            if (confirmation) {
                try {
                    await axios.delete(`http://localhost:5000/organisasi/${selectedOrganization.id_org}`);
                    const updatedOrgData = organisasi.filter(org => org.id_org !== selectedOrganization.id_org);
    ...
    const handleAddOrganizationClick = () => {
        // Menggunakan navigate untuk mengarahkan pengguna ke halaman tambah organisasi
        navigate("/add_organisasi");
    };
                ...
                <section id="editOrganisasi" className="editOrganisasi">
                    <div className="container">
                        {organisasi.length > 0 ? (
                                                    ...
                                                    {organisasi.map((org) => (
                                                        <li key={org.id_org}>
                                                            ...
                                                            <button className="btn btn-primary"
                                                                ...
                                                                style={{ backgroundColor: 'lightblue', color: 'black', marginBottom: '30px'}}
                                                            >
                                                                {selectedOrganization && org.id_org === selectedOrganization.id_org ? 'Unselect' : 'Select'}
                                                            </button>
                                                            {selectedOrganization && org.id_org === selectedOrganization.id_org && (
                                                                <button className="btn btn-primary"
                                                                    onClick={handleUpdateClick}
                                                                    style={{ backgroundColor: 'green', color: 'white', marginBottom: '30px' }}
                                                                >
                                                                    Update
                                                                </button>
                                                            )}
                                                            {selectedOrganization && org.id_org === selectedOrganization.id_org && (
                                                                <button className="btn btn-primary"
                                                                    ...
                                                                    style={{ backgroundColor: 'red', color: 'white', marginBottom: '30px' }}
                                                                >
                                                                    Delete
                                                                </button> 
                                                            )}
                            ...
                            <div>
                                <h3>Fill in organizational data</h3>
                                <div className="text-center mt-3">
                                    <button className="btn btn-dark mt-3" onClick={() => handleAddOrganizationClick()}>
                                        <i className="bx bx-plus"></i> Add Organization
                                    </button>
                                </div>
                            </div>
                        )}
                            ...
                            <div>
                                ...
                                <form>
                                    ...
                                    <div className="text-center">
                                        ...
                                        <button className="btn btn-primary" onClick={handleUpdateClick}>Simpan Perubahan</button>
                                        {updateSuccess && (
                                            <p>Data organisasi {selectedOrganization.nama_organisasi} berhasil diperbarui.</p>
                                        )}
                                    </div>
                                </form>
                            </div>
